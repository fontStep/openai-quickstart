...
def generate_cogview_image(prompt: str) -> str:
    ...
    print(f"generate_cogview_image response: {response}")
...
with st.container():
    ...
    with col1:
        ...
        st.text_input(label="图片风格",value="二次元风格",key="image_style",on_change=lambda : st.session_state["meta"].update(image_style=st.session_state["image_style"]),help="图片风格，默认为二次元风格")      
...
def draw_new_image():
    ...
    # TODO: 加上风格选项
    image_prompt = st.session_state["meta"]["image_style"] +"。"+ image_prompt.strip()
    ...
    for i in range(n_retry):
        try:
            ...
            print(f"generate_cogview_image error: {e}")
            if i < n_retry - 1:
